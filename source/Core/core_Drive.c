...
#include "core.h"
	...
	Int InomDef[dtNUM_OF-1]  	     = {13,11,18,52,52,47 ,56 ,110,85  ,148 ,240 ,294 ,358 ,574 ,840 ,11,9 ,13,32,32,33 ,73 ,85 ,95  ,150, 294, 170, 75, 18,33};		// default çíà÷åíèÿ äëÿ Inom äëÿ ðàçíûõ ïðèâîäîâ
	Int MomMaxDef[dtNUM_OF-1]  	     = {10,10,40,40,80,100,400,400,1000,1000,1000,1500,2000,3500,5000,10,10,40,40,80,100,400,400,1000,1000,1500,1000, 400, 200,400};	//				äëÿ Mmax
	Int TransCurrDef[14] 	 = {1500,1000,1000,1000,1000,1000,1100,1100,1100,700,300,400, 150, 1180};			//				äëÿ TransCur ïðàâèòü
	Int GearRatioDef[dtNUM_OF-1] 	 = {5250,5250,7560,5320,7560,7560,16720,16720,25146,25146,14400,14400,14400,16800,15600,5250,5250,7560,5320,7560,7560,16720,16720,25146,25146,14400,25146,16720, 2000,2000};		//äëÿ ïåðåäàòî÷íîãî ÷èñëà ðåäóêòîðà
	...
	void Core_Drive_Update(void)
	{
		...
		switch(g_Ram.ramGroupC.DriveType)
		{
			...
			case dt4000_EPZR:
						                PFUNC_blkRead(&drive30,         (Int *)(&g_Ram.ramGroupH.TqCurr),   LENGTH_TRQ);
						                PFUNC_blkRead(&TransCurrDef[13], (Int *)(&g_Ram.ramGroupH.TransCurr),          1);
						                break;
		    }

		if ((g_Ram.ramGroupC.DriveType < dtNUM_OF)&&(g_Ram.ramGroupC.DriveType != 0))
